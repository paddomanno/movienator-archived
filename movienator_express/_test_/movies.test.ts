...
async function createTestData() {
  ...
  const review1: Review = Review.create({
...
describe('MovieTests', () => {
  describe('Testing getAll Route', () => {
    it('Should return an array of all movies, ordered by title', async () => {
      const response = await request(app).get('/movie/all');
  ...
  describe('Testing getOne Route', () => {
    it('Should return a single movie', async () => {
      const response = await request(app).get('/movie/one/1');
      expect(response.statusCode).toBe(200);
      const movie: Movie = response.body.data as Movie;
    ...
    it('Should return 404 if the movies doesnt exist', async () => {
      const response = await request(app).get('/movie/one/10');
      expect(response.statusCode).toBe(404);
    });

    it('Should return 500 if the passed value is not a number', async () => {
      const response = await request(app).get('/movie/one/blabla');
  ...
  describe('Testing getMovieToActor Route', () => {
    it('Should return movies of that actor ordered by title', async () => {
      const response = await request(app).get('/movie/actor/1');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
    ...
    it("Should return 404 if the actor doesn't exist", async () => {
      const response = await request(app).get('/movie/actor/10');
      expect(response.statusCode).toBe(404);
    });

    it('Should return 500 if the passed value is not a number', async () => {
      const response = await request(app).get('/movie/actor/blabla');
      expect(response.statusCode).toBe(500);
    });
  });
  ...
  describe('Testing getReviewedByUser Route', () => {
    it('Should return the Reviewed Movies ordered by name', async () => {
      const response = await request(app).get('/movie/user/1');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
    ...
    it("Should return 404 if the movies doesn't exist", async () => {
      const response = await request(app).get('/movie/user/10');
      expect(response.statusCode).toBe(404);
    });

    it('Should return 500 if the passed value is not a number', async () => {
      const response = await request(app).get('/movie/user/blabla');
      expect(response.statusCode).toBe(500);
    });
  });
  ...
  describe('Testing getWatchlistByUser Route', () => {
    it('Should return all reviewed movies of that user ordered by title', async () => {
      const response = await request(app).get('/movie/watchlist/1');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(2);
      expect(movies.at(0).title).toBe('Aaaa');
    ...
    it('Should return 404 if the user doesnt exist', async () => {
      const response = await request(app).get('/movie/watchlist/10');
      expect(response.statusCode).toBe(404);
    });

    it('Should return 500 if the passed value is not a number', async () => {
      const response = await request(app).get('/movie/watchlist/blabla');
      expect(response.statusCode).toBe(500);
    });
  });
  ...
  describe('Testing moviesByMinTime Route', () => {
    it('Should return an array with movies ordered by title', async () => {
      const response = await request(app).get('/movie/time/min/200');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(2);
    ...
    it('Should return 500 if the param is zero', async () => {
      const response = await request(app).get('/movie/time/min/0');
      expect(response.statusCode).toBe(500);
    });

    it('Should return 500 if the passed value is not a number', async () => {
      const response = await request(app).get('/movie/time/min/blabla');
      expect(response.statusCode).toBe(500);
    });
  });
  ...
  describe('Testing moviesByMaxTime Route', () => {
    it('Should return array of movies ordered by title', async () => {
      const response = await request(app).get('/movie/time/max/200');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(2);
    ...
    it('Should return 500 if the param is zero', async () => {
      const response = await request(app).get('/movie/time/max/0');
      expect(response.statusCode).toBe(500);
    });
  ...
  describe('Testing moviesByDateRange Route', () => {
    it('Should return an array of movies ordered by title', async () => {
      const dateMin = new Date('2022-02-02');
      const dateMax = new Date('2022-06-06');
    ...
    it('Should return 500 if the second param is not a valid date', async () => {
      const dateMin = new Date('2022-02-02');
      const response = await request(app).get(`/movie/date/${dateMin}/blabla`);
      expect(response.statusCode).toBe(500);
    });

    it('Should return 500 if the first param is not a valid date', async () => {
      const dateMax = new Date('2022-02-02');
      const response = await request(app).get(`/movie/date/blabla/${dateMax}`);
      expect(response.statusCode).toBe(500);
    });
  ...
  describe('Testing moviesByWord Route', () => {
    it('Should return an array of movies ordered by title', async () => {
      const response = await request(app).get('/movie/name/b');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(2);
      expect(movies.at(0).movieId).toBe(2);
    ...
    it('Should return an empty array if no movies are found', async () => {
      const response = await request(app).get('/movie/name/xxx');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
  ...
  describe('Testing moviesByRating Route', () => {
    it('Should return an array of movies ordered by title', async () => {
...

      {
        ...
        await review1.save();
        const review2: Review = Review.create({
          reviewMovieMovieId: 1,
          reviewUserUserId: 2,
          title: 'Good',
          content: 'Was good',
        ...
        await review2.save();
        const review3: Review = Review.create({
          reviewMovieMovieId: 1,
          reviewUserUserId: 3,
          title: 'Good',
          content: 'Was good',
      ...
      //1 -> 2; 2 -> 3,3; 3 -> 5
      const response = await request(app).get('/movie/rating/3');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(2);
      expect(movies.at(0).movieId).toBe(2);
    ...
    it('Should return 500 if no number is passed', async () => {
      const response = await request(app).get('/movie/rating/blabla');
      expect(response.statusCode).toBe(500);
    });

    it('Should return 500 if a negative value is passed', async () => {
      const response = await request(app).get('/movie/rating/-1');
      expect(response.statusCode).toBe(500);
    });
  });
  ...
  describe('Testing getMoviesByGenre Route', () => {
    ...
    it('Should return an array of movies with that genre id', async () => {
      const response = await request(app).get('/movie/genre/1');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(1);
      expect(movies.at(0).actors.length).toBeGreaterThanOrEqual(1);
    ...
    it('Should return an array of movies with that genre name', async () => {
      const response = await request(app).get('/movie/genre/Fantasy');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(1);
      expect(movies.at(0).actors.length).toBeGreaterThanOrEqual(1);
    ...
    it('Should return 404 if that genre id doesnt exist', async () => {
      const response = await request(app).get('/movie/genre/10');
      expect(response.statusCode).toBe(404);
    });

    it('Should return 404 if that genre name doesnt exist', async () => {
      const response = await request(app).get('/movie/genre/blabla');
      expect(response.statusCode).toBe(404);
    });
  });
  ...
  describe('Testing getMutualWatchlist', () => {
    it('Should return an array of Movies', async () => {
      const response = await request(app).get('/movie/mutual/watchlist/1/2');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(1);
      expect(movies.at(0).movieId).toBe(1);
    ...
    it("Should return 404 if the first user doesn't exist", async () => {
      const response = await request(app).get('/movie/mutual/watchlist/100/2');
      expect(response.statusCode).toBe(404);
    });

    it("Should return 404 if the second user doesn't exist", async () => {
      const response = await request(app).get('/movie/mutual/watchlist/1/200');
      expect(response.statusCode).toBe(404);
    });

    it('Should return 500 if the first value is not a number', async () => {
      const response = await request(app).get(
        '/movie/mutual/watchlist/blabla/2'
      );
      expect(response.statusCode).toBe(500);
    });
    ...
    it('Should return 500 if the second value is not a number', async () => {
      const response = await request(app).get(
        '/movie/mutual/watchlist/1/blabla'
      );
      expect(response.statusCode).toBe(500);
    });
  });
  ...
  describe('Testing getMutualReview', () => {
    it('Should return an array of movies ordered by title', async () => {
      ...
      const response = await request(app).get('/movie/mutual/review/1/2');
      expect(response.statusCode).toBe(200);
      const movies: Movie[] = response.body.data as Movie[];
      expect(movies.length).toBe(1);
      expect(movies.at(0).movieId).toBe(1);
    ...
    it('Should return 404 if the first user doenst exist', async () => {
      const response = await request(app).get('/movie/mutual/review/100/2');
      expect(response.statusCode).toBe(404);
    });

    it('Should return 404 if the second user doenst exist', async () => {
      const response = await request(app).get('/movie/mutual/review/1/200');
      expect(response.statusCode).toBe(404);
    });

    it('Should return 500 if the first value is not a number', async () => {
      const response = await request(app).get('/movie/mutual/review/blabla/2');
      expect(response.statusCode).toBe(500);
    });

    it('Should return 500 if the second value is not a number', async () => {
      const response = await request(app).get('/movie/mutual/review/1/blabla');
      expect(response.statusCode).toBe(500);
    });
  });
  ...
  describe('Testing postMove Route', () => {
    it('Should insert the movie along with the actors and genres', async () => {
      ...
      const genre2: Genre = new Genre();
      genre2.genreId = 2;
      genre2.genreName = 'Krimi';

      const movie4: Movie = Movie.create({
        movieId: 4,
        title: 'Aaaa',
        adultContent: false,
        actors: [actor2],
      ...
      const response = await request(app).post('/movie/').send(movie4);
    ...
    it('Should return 500 if needed fields are missing', async () => {
      const movie5: Movie = Movie.create({
        movieId: 5,
        title: 'Aaaa',
      ...
      const response = await request(app).post('/movie/').send(movie5);
      expect(response.statusCode).toBe(500);
    });
  ...
  describe('Testing updateMovie Route', () => {
    it('Should update the movie but not the relations', async () => {
      ...
      const response = await request(app).put('/movie/').send(movie1);
      expect(response.statusCode).toBe(201);
